...
namespace HaloFrame
{
    ...
    public class CoroutineHelper : MonoBehaviour
    {
        ...
        public long AddDelayTask(float delay, Action action)
        {
            ...
            var delayTask = ReferencePool.Get<DelayTask>();
        ...
        public long AddConditionTask(Func<bool> condition, Action action, float timeout)
        {
            if (timeout <= 0)
                timeout = 0;
            var taskId = GenerateTaskId();
            var conditionTask = ReferencePool.Get<ConditionTask>();
        ...
        {
            var taskId = GenerateTaskId();
            var coroutineTask = ReferencePool.Get<CoroutineActionTask>();
        ...
        {
            var task = ReferencePool.Get<CoroutineTask>();
