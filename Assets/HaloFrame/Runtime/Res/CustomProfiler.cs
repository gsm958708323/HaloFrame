...
namespace HaloFrame
{
    public class CustomProfiler
    {
        ...
        private static readonly List<CustomProfiler> ms_Stack = new List<CustomProfiler>();
        

        private List<CustomProfiler> m_children;
        ...
        private CustomProfiler(string name, int level) : this(name)
        {
            m_Level = level;
        }

        public static CustomProfiler Create(string name)
        {
            return new CustomProfiler(name);
        }

        public CustomProfiler CreateChild(string name)
        {
            if (m_children == null)
            {
                m_children = new List<CustomProfiler>();
            }

            CustomProfiler profiler = new CustomProfiler(name, m_Level + 1);
        ...
        public void Stop()
        {
            if (m_Timestamp == -1)
            {
                throw new Exception($"{nameof(CustomProfiler)} {nameof(Stop)} error, repeat stop, name: {m_Name}");
            }
        ...
        public override string ToString()
        {
            ...
            while (ms_Stack.Count > 0)
            {
                ...
                CustomProfiler profiler = ms_Stack[index];
                ms_Stack.RemoveAt(index);

                profiler.Format();

                List<CustomProfiler> children = profiler.m_children;
                if (children == null)
                {
                    continue;
                }
